...
const defaultGeneratorOptions = { jsescOption: { minimal: true } };
...
const undefinedMatcher = m.or(
  m.identifier('undefined'),
  m.unaryExpression('void', t.numericLiteral(0)),
);
...
async function applyTransformAsync(ast, transform, options) {
  ...
  if (transform.visitor)
    traverseDefault(ast, transform.visitor(options), undefined, state);
      ...
      m.anyList(
        ...
        infiniteLoop(
          m.matcher((node) => {
            return (
              m
                .containerOf(m.callExpression(m.identifier('parseInt')))
                .match(node) &&
              m
                .blockStatement([
                  m.tryStatement(
  ...
  visitor() {
    ...
    const matcher = m.functionDeclaration(
      ...
      m.blockStatement([
        ...
        m.tryStatement(
          m.blockStatement([
            m.ifStatement( // Replaced ifStatement from @codemod/matchers with t.ifStatement for generation
              m.fromCapture(ret),
              m.blockStatement([
...
const UNESCAPE = typeof unescape === 'function' ? unescape : (str) => {
    // A basic polyfill for unescape, may not cover all edge cases like Node's util.unescape
    return String(str).replace(/%([0-9A-Fa-f]{2})/g, (match, p1) => String.fromCharCode(parseInt(p1, 16)))
...
const DECODE_URI = typeof decodeURI === 'function' ? decodeURI : (str) => {
    try { return decodeURIComponent(str.replace(/\+/g, ' ')); } catch { return str; } // Simplified
};
const DECODE_URI_COMPONENT = typeof decodeURIComponent === 'function' ? decodeURIComponent : (str) => {
    try { return global.decodeURIComponent(str); } catch { return str; } // Rely on global if available, else basic
};
...
const evaluateGlobalsTransform = {
  ...
  visitor() {
    const name = m.capture(
      m.or(...(Object.keys(FUNCTIONS_EVALUATE_GLOBALS))),
    );
    ...
    return {
      CallExpression: {
        exit(path) {
          ...
          try {
            const value = FUNCTIONS_EVALUATE_GLOBALS[name.current].call(
              globalThis,
              arg.current,
            );
...
const mergeObjectAssignmentsTransform = {
  ...
  visitor: () => {
    ...
    const repeatedCallMatcherMergeObj = m.or(
        m.forStatement(),
        m.forOfStatement(),
        m.forInStatement(),
        m.whileStatement(),
    ...
    const inlineableObjectMergeObj = m.matcher((node) =>
        m
        .or(
            safeLiteral,
            m.arrayExpression(m.arrayOf(inlineableObjectMergeObj)),
            m.objectExpression(m.arrayOf(constObjectProperty(inlineableObjectMergeObj))),
        )
        .match(node),
    );
        ...
        exit(path) {
          ...
          while (siblingIndex < container.length) {
            ...
            if (
              ...
              hasCircularReferenceMergeObj(value.current, binding)
            )
              return;
            const isComputed =
              computed.current &&
            ...
            sibling.remove();
            binding.dereference(); // Path was removed, so one less reference
            // Update referencePaths by removing the one that pointed to the removed sibling
            binding.referencePaths = binding.referencePaths.filter(p => p.parentPath !== sibling);

            if (
              binding.references === 1 &&
              inlineableObjectMergeObj.match(object.current) &&
              !isRepeatedCallReferenceMergeObj(binding, binding.referencePaths[0])
            ) {
  ...
  async run(ast, state, sandbox) {
    ...
    for (const decoder of decoders) {
      state.changes += applyTransform(
        ast,
        inlineDecoderWrappersTransform,
        decoder.path,
      ).changes;
    }
    const vm = new VMDecoder(sandbox, stringArray, decoders, rotator);
    state.changes += (
      await applyTransformAsync(ast, inlineDecodedStringsTransform, { vm })
    ).changes;
    ...
    state.changes += applyTransforms(
      ast,
      [mergeStringsTransform, deadCodeTransform, controlFlowObjectTransform, controlFlowSwitchTransform],
      { noScope: true },
    ).changes;
  },
};
...
const invertBooleanLogicTransform = {
  ...
  visitor: () => {
    const logicalExpression = m.logicalExpression(
      m.or(...Object.values(INVERTED_LOGICAL_OPERATORS_UNMINIFY)),
    );
    const logicalMatcher = m.unaryExpression('!', logicalExpression);
    const binaryExpression = m.capture(
      m.binaryExpression(m.or(...Object.values(INVERTED_BINARY_OPERATORS_UNMINIFY))),
    );
        ...
        exit(path) {
          ...
          if (binaryMatcher.match(path.node)) {
            binaryExpression.current.operator =
              INVERTED_BINARY_OPERATORS_UNMINIFY[
          ...
          while (logicalExpression.match(current)) {
            current.operator =
              INVERTED_LOGICAL_OPERATORS_UNMINIFY[
                current.operator
              ];
        ...
        exit(path) {
          if (
            ...
            try {
              ...
              const parsed = parse(string.current, { allowReturnOutsideFunction: true }).program.body[0].expression;
              path.replaceWith(parsed);
              this.changes++;
            } catch {
              // JSON or expression parsing failed
            }
          }
        },
      },
...
const numberExpressionsMatcher = m.or(
...
const removeDoubleNotTransform = {
  ...
  visitor() {
    ...
    const doubleNot = m.unaryExpression(
      '!',
      m.unaryExpression('!', expressionMatcher),
    );
...
const splitForLoopVarsMatcher = m.forStatement(
    ...
    return {
      BinaryExpression: {
        exit(path) {
          ...
          // No this.changes++ as it's a mutation, not a replacement.
          // Or, if we want to count it:
          // this.changes++;
        },
      },
    };
    ...
    return {
      BinaryExpression: {
        exit(path) {
          ...
          path.replaceWith(
            t.binaryExpression(
              TYPEOF_UNDEFINED_OPERATOR_MAP[operator.current],
...
const unminifyBooleansTrueMatcher = m.or(
  m.unaryExpression('!', m.numericLiteral(0)),
  m.unaryExpression('!', m.unaryExpression('!', m.numericLiteral(1))),
  m.unaryExpression('!', m.unaryExpression('!', m.arrayExpression([]))),
);
const unminifyBooleansFalseMatcher = m.or(
  m.unaryExpression('!', m.numericLiteral(1)),
  m.unaryExpression('!', m.arrayExpression([])),
);
...
const yodaTransform = {
  ...
  visitor: () => {
    ...
    const matcher = m.binaryExpression(
      m.or(...Object.values(FLIPPED_YODA_OPERATORS)),
      pureValue,
...
class UnpackModule { // Renamed to avoid clash with Node's Module
  constructor(id, ast, isEntry) {
    this.id = id;
    this.ast = ast;
    this.isEntry = isEntry;
    this.path = `./${isEntry ? 'index' : id.replace(/\.js$/, '')}.js`;
    this._code = undefined; // Private field name convention
  }
  regenerateCode() {
...
class Bundle {
  ...
  async save(bundlePath) { // Renamed 'path' to 'bundlePath' to avoid conflict with 'nodePath'
    const bundleJson = {
      type: this.type,
    ...
    // const { mkdir, writeFile } = await import('node:fs/promises'); // Already imported at top
    await nodeMkdir(bundlePath, { recursive: true });
    await nodeWriteFile(
      nodePath.join(bundlePath, 'bundle.json'),
      JSON.stringify(bundleJson, null, 2),
      'utf8',
    );
    await Promise.all(
      Array.from(this.modules.values(), async (module) => {
        const modulePath = nodePath.normalize(nodePath.join(bundlePath, module.path));
        if (nodePath.relative(bundlePath, modulePath).startsWith('..')) {
          throw new Error(`detected path traversal: ${module.path}`);
        }
        await nodeMkdir(nodePath.dirname(modulePath), { recursive: true });
        await nodeWriteFile(modulePath, module.code, 'utf8');
      }),
    );
  }
  applyTransforms() {} // Placeholder, specific bundles override this
}

// File: unpack/path.ts
const posixPath = nodePath.posix; // Use posix for consistent path handling
...
function resolveDependencyTree(tree, entry) {
  ...
  return Object.fromEntries(
    Object.entries(paths).map(([id, p]) => {
      const newPath = p.startsWith('node_modules/')
        ? p
        : posixPath.join(prefix, p);
      return [id, newPath];
    }),
  );
}
function resolveTreePaths(
  ...
  const entries = Object.entries(graph[entry] || {}); // Handle cases where entry might not be in graph or have deps
  for (const [id, name] of entries) {
    const isCircular = Object.hasOwn(paths, id);
    if (isCircular) continue;
    let currentPath; // Renamed to avoid conflict with 'path' module
    if (name.startsWith('.')) {
...
function webpackRequireFunctionMatcher() {
  ...
  const webpackRequire = m.capture(
    m.functionDeclaration(
      m.identifier(), // Can be anonymous or named
      [m.identifier()],
      m.blockStatement(
        anySubList(
          m.expressionStatement(
              ...
              m.or(
                constMemberExpression(
                  m.memberExpression(
                    ...
                    m.identifier(), // property can be anything
                    true, // computed (e.g. __webpack_modules__[moduleId])
                  ),
                  'call',
                ),
                m.memberExpression( // For cases without .call (direct function call)
                  m.fromCapture(containerId),
                  m.identifier(),
                  true,
                ),
              ),
              // Arguments can vary, typically [module, module.exports, __webpack_require__]
            ),
          ),
        ),
      ),
...
function modulesContainerMatcher() {
  return m.capture(
    m.or(
      m.arrayExpression(m.arrayOf(m.or(anonymousFunction(), m.nullLiteral()))), // webpack 4 often uses arrays, null for non-existent modules
      m.objectExpression( // webpack 5+ often uses objects
        m.arrayOf(
          m.or(
            m.objectProperty( // { moduleId: function(...) {} }
              m.or(m.numericLiteral(), m.stringLiteral(), m.identifier()),
              anonymousFunction(),
            ),
             m.objectProperty(m.identifier('c'), m.stringLiteral()) // Example of other properties like comments, ignore for module extraction
          ),
        ),
      ),
    ),
...
function findAssignedEntryId(webpackRequireBinding) {
  ...
  const assignment = m.assignmentExpression(
    '=',
    constMemberExpression(webpackRequireBinding.identifier.name, 's'), // .s is often used for entry module id
    entryId,
  );
...
function findRequiredEntryId(webpackRequireBinding) {
  ...
  // Look for the first call to __webpack_require__ that is not part of its own definition or internal helpers
  for (const reference of webpackRequireBinding.referencePaths) {
    // Ensure the reference is the callee of a call expression
    if (reference.key === 'callee' && call.match(reference.parent)) {
        // Additional checks can be added here if needed to distinguish entry call from internal calls
        return String(entryId.current.value);
    }
  }
}
...
function inlineVarInjections(module) {
  ...
  for (const node of program.body) {
    if (matcher.match(node)) {
      const vars = params.current.map((param, i) =>
        buildVarInjectionVar({ NAME: param, INIT: args.current[i + 1] }), // args[0] is 'this', so params align with args[1+]
      );
      newBody.push(...vars);
...
function convertESM(module) {
  const defineEsModuleMatcher = m.expressionStatement(
    m.callExpression(constMemberExpression(m.identifierType('Identifier'), 'r'), [m.identifier()]), // require.r or __webpack_require__.r
  );
  ...
  const defineExportMatcher = m.expressionStatement(
    m.callExpression(constMemberExpression(m.identifierType('Identifier'), 'd'), [ // require.d or __webpack_require__.d
      exportsName,
  ...
  const defineExportsMatcher = m.expressionStatement(
    m.callExpression(constMemberExpression(m.identifierType('Identifier'), 'd'), [
      exportsName,
      m.objectExpression(properties),
    ]),
  );
  const requireVariable = m.capture(m.identifier());
  const requiredModuleId = m.capture(m.anyNumber()); // Can be string too
  const requireMatcher = m.variableDeclaration(undefined, [
    m.variableDeclarator(
      requireVariable,
      m.callExpression(m.identifierType('Identifier'), [ // require() or __webpack_require__()
  ...
  const hmdMatcher = m.expressionStatement(
    m.assignmentExpression(
      '=',
      m.identifier('module'),
      m.callExpression(constMemberExpression(m.identifierType('Identifier'), 'hmd')),
    ),
  );
  ...
  function exportVariableEsm(requireDPath, value, exportNameStr) {
    if (value.type === 'Identifier') {
        ...
        declaration.replaceWith(t.exportNamedDeclaration(declaration.node, [])); // Ensure no specifiers if it's a direct export of declaration
        }
      return;
    }
    if (exportNameStr === 'default') {
        ...
        buildNamedExportLet({ NAME: t.identifier(exportNameStr), VALUE: value }),
        );
    }
  }
  ...
  traverseDefault(module.ast, {
    enter(path) {
      if (path.parentPath?.parentPath && !path.isProgram()) return path.skip(); // Allow Program to be processed
      ...
      if (defineEsModuleMatcher.match(path.node)) {
        ...
        // Ensure the callee is 'require' or '__webpack_require__'
        (path.node.declarations[0].init.callee.name === 'require' || path.node.declarations[0].init.callee.name === module.webpackRequire)
      ) {
        path.replaceWith(
          buildNamespaceImport({
            NAME: requireVariable.current,
            PATH: String(requiredModuleId.current.value), // Use .value for literals
          }),
        );
        ...
        const emptyObject = emptyObjectVarMatcher.match(
          ...
          ? (exportsBinding?.path.node.init)
          : null;
        for (const property of properties.current) {
          const exportedKey = property.key;
          const returnedValueFromArrow = (property.value)
            .body;
          if (emptyObject) {
            emptyObject.properties.push(
              t.objectProperty(exportedKey, returnedValueFromArrow),
            );
          } else {
...
function convertDefaultRequire(bundle) {
  ...
  const declaratorMatcher = m.variableDeclarator(
    m.identifier(), // Any variable name
    m.callExpression(m.identifierType('Identifier'), [requiredModuleId]), // Callee can be 'require' or '__webpack_require__'
  );
  const moduleArg = m.capture(m.identifier());
  const getterVarName = m.capture(m.identifier());
  const requireN = m.callExpression(constMemberExpression(m.identifierType('Identifier'), 'n'), [ // require.n or __webpack_require__.n
    moduleArg,
  ]);
  const defaultRequireMatcher = m.variableDeclarator(getterVarName, requireN);
  const defaultRequireMatcherAlternative = m.or(
    constMemberExpression(requireN, 'a'), // .a typically means get default export
    m.callExpression(requireN, []),      // Calling the getter itself
  );
  ...
  function getRequiredModuleDefault(path) {
    ...
    // Ensure callee of declarator init is 'require' or the bundle's webpackRequire
      ...
      'CallExpression|MemberExpression'(path) {
        if (defaultRequireMatcherAlternative.match(path.node)) {
          ...
          if (requiredModule?.ast.program.sourceType === 'module') {
            path.replaceWith(
              buildDefaultAccessGetDefaultExport({ OBJECT: moduleArg.current }),
            );
          } else {
...
class WebpackBundle extends Bundle {
  ...
  applyTransforms() {
    // Assign webpackRequire name to each module for use in ESM conversion
    this.modules.forEach(module => {
        const { webpackRequire } = webpackRequireFunctionMatcher();
        traverseDefault(module.ast, {
            FunctionDeclaration(path) {
                if (webpackRequire.match(path.node)) {
                    module.webpackRequire = path.node.id.name;
                    path.stop();
                }
            },
            noScope: true
        });
        // Fallback if not found (e.g. chunk or different structure)
        if (!module.webpackRequire) module.webpackRequire = '__webpack_require__';
    });

    this.modules.forEach(inlineVarInjections);
    this.modules.forEach(convertESM); // Needs module.webpackRequire to be set
    convertDefaultRequire(this); // Also needs module.webpackRequire for context
    this.replaceRequirePaths();
  }
  replaceRequirePaths() {
    const requireId = m.capture(m.or(m.numericLiteral(), m.stringLiteral()));
    const requireMatcher = m.callExpression(m.identifierType('Identifier'), [requireId]); // require or __webpack_require__
    ...
    this.modules.forEach((module) => {
      traverseDefault(module.ast, {
        'CallExpression|ImportDeclaration': (path) => {
          ...
          if (requireMatcher.match(path.node) && (path.node.callee.name === 'require' || path.node.callee.name === module.webpackRequire)) {
          ...
          if (isWebpackRequireCall && !/^\d+$/.test(moduleIdStr) && !/^"\d+"$/.test(moduleIdStr)) {
             // Not a typical webpack module ID require (e.g. require('fs')), skip.
             return;
          }
          
          const requiredModule = this.modules.get(moduleIdStr);
          const replacementPath = relativePath(
            module.path,
            requiredModule?.path ?? `./${moduleIdStr}.js`,
          );
...
const unpackWebpack4Transform = {
  ...
  visitor(options = { bundle: undefined }) {
    ...
    // Webpack 4: (function(modules) { ... })([modules]) or ({modules})
    const matcher = m.callExpression(
      m.functionExpression(
        null, // Can be anonymous
        [containerId], // Argument name for modules container
        m.blockStatement(anySubList(webpackRequire)), // Function body containing __webpack_require__ definition
      ),
      [container], // Call with the actual modules container
    );
    ...
    return {
      CallExpression(path) {
        ...
        for (const [id, funcPath] of getModuleFunctions(containerPath)) {
          renameParameters(funcPath, ['module', 'exports', 'require']);
          const isEntry = id === entryId;
          const file = t.file(t.program(funcPath.node.body.body));
          // Remove webpack's sourceURL comments if they exist, e.g. "//# sourceURL=webpack:///./src/index.js?");
...
const unpackWebpack5Transform = {
  ...
  visitor(options = { bundle: undefined }) {
    const { webpackRequire, containerId } = webpackRequireFunctionMatcher();
    const container = modulesContainerMatcher();
    // Webpack 5: var modules = { ... }; var __webpack_require__ = function ...;
    const matcher = m.blockStatement( // Or Program node
      anySubList(
        m.variableDeclaration(undefined, [ // `var __webpack_modules__ = ...` or `const __webpack_modules__ = ...` etc.
          m.variableDeclarator(containerId, container),
        ]),
        webpackRequire, // __webpack_require__ function declaration
      ),
    );

    return {
      'BlockStatement|Program'(path) { // Matcher can be in a block or at top level of program
        if (!matcher.match(path.node)) return;
        
        // Need to ensure the captured containerId and webpackRequire.current.id.name are from the same scope
        // and correctly associated by the matcher logic.
        // This might require refining the matcher or how bindings are checked.
        ...
        // Ensure the found declarator and function are indeed what `containerId` and `webpackRequire` captured
        if (containerBinding.path.get('init').node !== container.current || 
            webpackRequireBinding.path.node !== webpackRequire.current) {
            // If current captures don't align with bindings, could be a coincidental match.
            // This check helps ensure we're on the right __webpack_modules__ and __webpack_require__.
            return;
        }
        ...
        const containerPath = containerBinding.path.get('init'); // Path to the actual modules object/array node
        const modules = new Map();
...
const unpackWebpackChunkTransform = {
  ...
  visitor(options = { bundle: undefined }) {
    ...
    const jsonpGlobal = m.capture(
      constMemberExpression(
        m.or(m.identifier(), m.thisExpression()), // self, window, or an identifier
        m.matcher((property) => typeof property === 'string' && property.startsWith('webpack')), // e.g., self.webpackChunk_name_
      ),
    );
    const chunkIds = m.capture( // Array of chunk IDs, e.g. ["src_bootstrap_js"]
      m.arrayOf(m.or(m.numericLiteral(), m.stringLiteral())),
    );
    const matcher = m.callExpression(
      constMemberExpression( // (self.webpackChunk_name_ = self.webpackChunk_name_ || []).push(...)
        m.assignmentExpression(
          '=',
          jsonpGlobal,
          m.logicalExpression(
      ...
      [ // Arguments to push: [[chunkIds], modules, entryModulesArray?]
        m.arrayExpression(
          m.anyList( // The outer array passed to .push
            m.arrayExpression(chunkIds), // First element: array of chunkId strings/numbers
            container, // Second element: the modules object/array
            m.zeroOrMore(), // Optional third element (entry point execution, etc.)
          ),
        ),
      ],
    );
    ...
    return {
      CallExpression(path) {
        ...
        const modules = new Map();
        // The container is the second element in the array pushed to webpackChunk.push
        // path.get('arguments.0.elements.1') should get the module container node path
        const containerPath = path.get(container.currentKeys.join('.'));


        for (const [id, funcPath] of getModuleFunctions(containerPath)) {
          renameParameters(funcPath, ['module', 'exports', 'require']);
          const isEntry = false; // Chunks usually don't define the main entry point this way
          const file = t.file(t.program(funcPath.node.body.body));
          modules.set(id, new WebpackModule(id, file, isEntry));
        }
        // Entry ID is not typically found in chunks this way, so pass empty or handle differently if needed
...
function unpackAST(ast, mappings = {}) {
  ...
  traverseDefault(ast, visitor, undefined, { changes: 0 }); // Assuming state isn't strictly needed for these unpackers
  if (options.bundle) {
    options.bundle.applyMappings(mappings);
    options.bundle.applyTransforms(); // This will call WebpackBundle's applyTransforms
    unpackWebcrackLogger(
...
const jsxNewTransform = {
  ...
  visitor: () => {
    ...
    const typeCapture = m.capture(m.anyExpression()); // Renamed to avoid conflict
    const fragmentType = constMemberExpression('React', 'Fragment');
    const propsCapture = m.capture(m.objectExpression()); // Renamed
    const keyCapture = m.capture(m.anyExpression()); // Renamed
    ...
    const jsxMatcher = m.callExpression(
      m.or(
        ...
        m.sequenceExpression([ // (0, _core.jsx)()
          m.numericLiteral(0), 
          constMemberExpression(m.identifier(), jsxFunction)
        ]),
        constMemberExpression(m.identifier(), jsxFunction), // React.jsx()
      ),
      m.anyList(typeCapture, propsCapture, m.slice({ min: 0, max: 1, matcher: keyCapture })),
    );
    ...
    function convertAttributeValueJsxNew(expressionNode) {
        if (expressionNode.type === 'StringLiteral') {
            const hasSpecialChars = /["\\]/.test(expressionNode.value); // Simplified check for quotes/backslashes
    ...
    function convertChildrenJsxNew(object, pragma) {
        ...
        if (pragma.includes('jsxs') && t.isArrayExpression(childrenCapture.current)) {
            return childrenCapture.current.elements.map((child) =>
                convertChildJsxNew(child),
            );
        }
        return [convertChildJsxNew(childrenCapture.current)];
    }
    ...
    return {
      CallExpression: {
        exit(path) {
          ...
          if (
            t.isIdentifier(typeCapture.current) &&
            /^[a-z]/.test(typeCapture.current.name) // Host component like 'div'
          ) {
            // If it's a variable that happens to be lowercase, rename it
            const binding = path.scope.getBinding(typeCapture.current.name);
            if (binding && !binding.path.isImportSpecifier() && !binding.path.isImportDefaultSpecifier()) { // Don't rename imports
                ...
                nameNode = t.jsxIdentifier(newNameForComponent);
                 ...
                 // It's a string literal for a host component, or an import that shouldn't be renamed.
                 // nameNode should already be correctly t.jsxIdentifier(typeCapture.current.name)
            }
          }
          
          const attributes = convertAttributesJsxNew(propsCapture.current);
          if (path.node.arguments.length === 3 && keyCapture.current) { // Key is present
            attributes.push(
              t.jsxAttribute(
                t.jsxIdentifier('key'),
                convertAttributeValueJsxNew(keyCapture.current),
              ),
            );
          }
          
          const children = convertChildrenJsxNew(
            propsCapture.current,
            jsxFunction.current,
          );
          ...
          path.node.leadingComments = null; // Remove comments from the original call
...
const jsxTransform = {
  ...
  visitor: () => {
    ...
    const typeCapture = m.capture( // Renamed
      m.or(
        m.identifier(),
        m.stringLiteral(),
        deepIdentifierMemberExpression,
    ...
    const propsCapture = m.capture(m.or(m.objectExpression(), m.nullLiteral())); // Renamed
      ...
      CallExpression: {
        exit(path) {
          ...
          if (fragmentMatcher.match(path.node)) {
            const children = convertChildrenJsx(
            ...
            if (
              t.isIdentifier(typeCapture.current) &&
              /^[a-z]/.test(typeCapture.current.name)
            ) {
              const binding = path.scope.getBinding(typeCapture.current.name);
              if (binding && !binding.path.isImportSpecifier() && !binding.path.isImportDefaultSpecifier()) { // Don't rename imports
                const newNameForComponent = generateUid(path.scope, 'Component');
            ...
            const attributes = t.isObjectExpression(propsCapture.current)
              ? convertAttributesJsx(propsCapture.current)
              : [];
            const children = convertChildrenJsx(
              path.node.arguments.slice(2),
            );
...
const requireMatcherMangle = m.variableDeclarator(
  m.identifier(),
...
function inferNameMangle(path) {
  if (path.parentPath.isClass({ id: path.node })) {
    ...
    return path.node.name; // Fallback, should ideally not happen for mangling
  }
}
function generateExpressionNameMangle(expressionPath) {
  if (expressionPath.isIdentifier()) {
    ...
    return generateExpressionNameMangle(
      expressionPath.get('callee'),
    );
...
const mangleTransform = {
  ...
  visitor(match = () => true) {
    return {
      BindingIdentifier: { // Use BindingIdentifier to target variable/function/class names at their declaration
        exit(path) {
          // Check if it's a binding identifier (not a reference)
          // path.isBindingIdentifier() is implicitly true due to key 'BindingIdentifier'
          
          // Skip import specifiers
          if (path.parentPath.isImportSpecifier() || 
              path.parentPath.isImportDefaultSpecifier() || 
              path.parentPath.isImportNamespaceSpecifier()) return;

          // Skip object properties that are shorthand or keys (handled by other transforms if needed)
          if (path.parentPath.isObjectProperty() && path.key === 'key' && !path.parentPath.node.computed) return;
          // Skip class method names, property names (unless computed and identifier)
          ...
          // Don't rename if it's part of an export statement
          ...
          // Check if the binding is for a top-level declaration that might be implicitly exported or globally relevant
          if (binding.scope.isProgramScope() && (t.isFunctionDeclaration(binding.path.parent) || t.isClassDeclaration(binding.path.parent) || (t.isVariableDeclarator(binding.path.node) && binding.path.parentPath.parentPath.isProgram()))) {
             // Potentially don't mangle top-level items unless explicitly told it's safe
             // For now, let's assume `match` handles this safety.
          }

          renameFast(binding, inferNameMangle(path));
          this.changes++; // Count change if rename happened
        },
      },
    };
  },
...
function runPlugins(ast, plugins, state) {
  ...
  // Promisify pre/post if they are synchronous
  const runPre = async () => {
...
// File: index.ts (webcrack function)
const webcrackLogger = debugLib('webcrack:main'); // Use a different namespace for the main logger

async function webcrack(code, options = {}) {
  // Simplified mergeOptions as it's internal now
  const mergedOptions = {
    jsx: true,
    unminify: true,
    unpack: true,
  ...
  options = mergedOptions; // For internal use
  ...
  if (isBookmarklet) {
    ...
    // More robust decoding for bookmarklets that might have mixed encoding:
    // Split by '%', decode parts that are valid URI components, rejoin.
    // This is a simplification; proper mixed encoding handling is complex.
    // The original code `split(/%(?![a-f\d]{2})/i).map(decodeURIComponent).join('%')`
    // was an attempt to handle this. For simplicity, using a single decode.
  }

  let ast;
  let outputCode = '';
  let bundle;
  const { plugins: customPlugins } = options; // Renamed to avoid conflict
  const state = { opts: {} }; // Plugin state
  ...
  const stages = [
    () => {
      ast = parse(code, {
        ...
        errorRecovery: true, // Keep error recovery
    ...
    () => {
      applyTransforms(
        ast,
        [blockStatementsTransform, sequenceTransform, splitVariableDeclarationsTransform, varFunctionsTransform],
        { name: 'prepare' },
      );
    },
    ...
    options.unminify && (() => {
      applyTransforms(ast, [transpileTransform, unminifyTransform]);
    }),
    ...
    options.deobfuscate && (() => applyTransforms(ast, [mergeObjectAssignmentsTransform, evaluateGlobalsTransform])),
    () => (outputCode = generate(ast)), // Use the generate from ast-utils
  ...
  return {
    code: outputCode,
    bundle,
    async save(savePath) { // Renamed path to savePath
      // const { mkdir, writeFile } = await import('node:fs/promises'); // Already imported
      const normalizedPath = nodePath.normalize(savePath);
      await nodeMkdir(normalizedPath, { recursive: true });
      await nodeWriteFile(nodePath.join(normalizedPath, 'deobfuscated.js'), outputCode, 'utf8');
      if (bundle) { // Check if bundle is defined
...
async function runCli() {
  ...
  try {
    // __dirname will be the directory of the merged script.
    // This assumes package.json is one level up from where the script is run or installed.
    // This might be fragile depending on deployment.
    const __filename = nodeURL.fileURLToPath(import.meta.url); // For ES module like behavior if script is run as module
  ...
  debugLib.enable('webcrack:*'); // Enable debug for CLI by default
  ...
  commanderProgram
    ...
    .action(async (inputFile) => { // Renamed input to inputFile
      const { output: outputDir, force, ...options } = commanderProgram.opts(); // Renamed output to outputDir
      const code = await (inputFile ? nodeReadFile(inputFile, 'utf8') : readStdin());
      
      if (outputDir) {
        if (force || !nodeFS.existsSync(outputDir)) {
          await nodeRm(outputDir, { recursive: true, force: true });
        } else {
          commanderProgram.error('output directory already exists'); // exits
        }
      }
      
      // Map CLI options to webcrack options
      const webcrackOptions = {
        mangle: options.mangle,
        jsx: options.jsx, // Commander sets it to false if --no-jsx
        unpack: options.unpack,
        deobfuscate: options.deobfuscate,
        unminify: options.unminify,
        // sandbox and onProgress will use defaults from webcrack
      };
...
// Exports for library usage (if needed, e.g. CJS)
// module.exports = { webcrack, createNodeSandbox, createBrowserSandbox };
// For ES module style, if this script itself is treated as a module:
// export { webcrack, createNodeSandbox, createBrowserSandbox };
...
// To make it directly usable as a library that can be required:
module.exports = {
    webcrack,
    createNodeSandbox, // Exporting sandbox creation utils
    createBrowserSandbox,
    // Potentially export other useful things if they were top-level exports before
    // For example, individual transforms if they were meant to be reusable.
    // For now, just the main entry points.
}; 
